...
function parseIndicators(data: AxiosResponse): Indicators[] {
  ...
  if (dataArr === undefined) {
    error('Error while parsing array of Indicators: not an array', undefined, {data});
    return [];
  }
  const parsedIndicators = dataArr.map(item => {
    ...
    return {marker, Indicators: theIndicators, Send, Needrunning, Addup, Stopper, Needtohave, Ignore};
  });
...
function parseDates(data: AxiosResponse): Map<string, string> {
  ...
  if (stringMap === undefined) {
    error('Error while parsing a map of dates, not an object', undefined, {data});
