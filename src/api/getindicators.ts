...
function parseIndicators(data: AxiosResponse): Indicators[] {
  ...
  const parsedIndicators = dataArr.map(item => {
    ...
    if (itemMap === undefined) {
      error(
        "Error while parsing an Indicators: value is not an object",
        undefined,
        { item }
      );
      return undefined;
...
export async function getindicators(
  ...
  const indicators = parseIndicators(
    await Request.get(`mtg/donew2.php?cmd=cm_getindicators&version=${version}`)
  );
  const dates = parseDates(
    await Request.get(`mtg/donew2.php?cmd=cm_getdateformats&version=${version}`)
  );
